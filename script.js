  ...
  if (audio) {
    ...
    if (tryPlay && typeof tryPlay.then === "function") {
      tryPlay
        ...
        .catch(() => { setPlaying(false); musicMissingHint(); }); // bloqueado OU arquivo ausente
    }
  }
...
// Avisa, de forma discreta, que o arquivo da música ainda não foi adicionado
function musicMissingHint() {
  const artist = $(".player__artist");
  if (artist) artist.textContent = "adicione o arquivo her.mp3";
}
...
function setupPlayer() {
  ...
  audio?.addEventListener("error", () => musicMissingHint());
}
